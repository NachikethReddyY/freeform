...
- [x] Add the local board dashboard, create, inline rename, name-only search, collections, per-card and bulk moves, and recoverable Trash. Browser proof covers create/open/back, search/select, collection creation/filtering/rename, card move, a two-board bulk move, Copy link, collection-rename cancel, delete to Trash, restore/open with the title intact, and hard reload without the reported Hook crash; 20 board tests pass. The original crash cause is not identified.
- [x] Store bounded board preview thumbnails in origin-local IndexedDB for opened boards; hook and model tests pass. Text-only thumbnails now wait for fonts and retain valid fractional image dimensions. A hard-reloaded browser card shows `Regular Bold` with distinct weight; shape previews remain visible. Old or never-opened boards show the FreeForm icon until opened.
...
- [x] Add five editable native starters: flowchart, mind map, ERD, sequence, and architecture. The Sequence starter uses three participant headers, native bound lifelines, and four chronological bound messages. Browser insertion, whole-diagram drag and hard reload passed; focused sequence tests cover layout, hidden anchors and movement. Flowchart insertion also passed in the browser.
- [x] Offer a compact Arrange action for a fully connected selection of native geo/note nodes. Browser Flowchart passes showed readable branch labels, one-step undo, reload persistence and all selected nodes visible beside the style panel with bound arrows attached. Eight focused layout tests cover bindings, labels, cycles, no-op, unavailable selections, unrelated-shape clearance, frame backdrops and viewport fit. Small-anchor and disconnected authored layouts, such as Sequence, do not offer the action. Unrelated-shape/frame collision has focused-test proof only; large-board performance is unmeasured.
