...
- [x] Add five editable native starters: flowchart, mind map, ERD, sequence, and architecture. The Sequence starter now uses three participant headers and native bound lifelines, plus four chronological bound messages; browser insertion/reload and two focused sequence tests cover it. Flowchart insertion also passed in the browser.
