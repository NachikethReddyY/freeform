...
- [x] Add the local board dashboard, create, inline rename, name-only search, collections, per-card and bulk moves, and recoverable Trash. Browser proof covers create/open/back, search/select, collection creation/filtering/rename, card move, delete to Trash, restore/open with the title intact, and hard reload without the reported Hook crash; 19 board tests pass. The original crash cause is not identified.
...
- [ ] Finish final editor chrome and typography proof across remaining states and widths. Rectangle label S→XL and reload passed; a selected-shape browser pass covered width, dash, rounded edge, shape picker, opacity, Duplicate/Delete icons, Text S→XL, bold and More. Bold is heavier in editor/SVG with PNG/thumbnail proof pending. The dashboard passed 720px and card-menu checks; independent Stroke/Background color and some theme/mobile states remain open.
...
- [x] Add compact board-content search for native text/geo/note/arrow labels across pages; browser proof covers no match, clear/Escape, and jumping to/selecting/centering a match on another page. Results are capped at 100 visible items.
