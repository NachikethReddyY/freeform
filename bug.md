...
| Focus mode | Safari enter/exit pass | Cmd+K exposed Focus mode, hid editor and FreeForm floating chrome, then restored board controls on exit; palette tests pass 9/9 | AI draft retention across Focus mode and other viewport states were not separately checked |
| Layers navigation | Four-row Safari pass | Current-page native hierarchy opened; choosing Slide 1 selected and centered the frame. Three focused tests cover hierarchy/selection without board mutation. | Nested row selection, large boards and narrow layouts remain unverified. |
...
| Slides | First slide, direct laser, remote return and rapid Next pass | The stage fills the browser tab by default with explicit browser fullscreen. Helium captured light/dark slides; Safari showed nonblank slides, Next/Previous and a blue laser trail. In a clean two-tab Safari run the remote connected, Next advanced to 2/2, and the presenter stage remained 2/2 on return. On a separate three-frame board, two quick Next actions showed Slide 3 during and after the move. Auth tests pass 8/8, presentation tests 29/29. | Remote touchpad movement and broader transition appearance remain unverified. No live audience sharing claim. |
...
| Blue application accent | Sign-in, dashboard and direct laser Safari views pass | The account screen shows a blue primary button/focus treatment; the signed-in dashboard has blue navigation selection and Start drawing, and the editor shell has blue selection. A direct Safari laser gesture showed a short fading blue trail. | Remote laser touchpad color remains unverified. Canvas palette intentionally includes violet drawing swatches. |
...
### B29: Rapid slide navigation could drop a frame mid-transition

- [x] **Keep traversed slides visible until the camera finishes moving.**
- **Observed risk:** a second quick Next replaced the single previous-frame ID before the first 440 ms camera move ended, so its slide could disappear during the move.
- **Fix and proof:** the presenter retains every frame traversed during rapid navigation until 480 ms after the last move. A failed-first regression passes in the 29/29 presentation suite, and the build passes. On a disposable three-frame Safari board, two quick Next actions showed Slide 3 during and after the move without a blank stage.
- **Limit:** one rapid three-frame browser path was checked. This does not prove every transition is smooth or that the remote touchpad works.

### B27 — Presenter view persistence across remote tabs

- [x] **Keep the presenter visible when returning from its second-tab remote.**
- **Cause:** the auth focus recheck briefly changed to loading and unmounted the board, ending its presentation when the remote tab gained focus.
- **Fix and proof:** a same-owner board stays mounted but hidden and inert during recheck, then resumes only after valid owner confirmation. The AI chat portal now renders inside that wrapper; Safari confirmed the panel still opens. The presenter remote reports no active slide and ignores commands while recheck is pending. Invalid or different-owner sessions unmount the board. Presentation starts in a tab-filling stage; browser fullscreen is explicit. In a clean two-tab Safari run the remote showed Connected, Next advanced to 2/2, and the returning presenter stage still showed slide 2/2. Auth tests pass 8/8, presentation tests 27/27 and the build passes.
- **Limit:** one clean two-tab Safari session was sampled. The pending-command guard has source review, not a direct browser pass; this is not a full authentication audit. Remote touchpad movement and transition appearance are separate open checks.

### B28 — Copied URL fell through card review

- [x] **Recognize a safe URL when the clipboard includes matching plain text and URI-list representations.**
- **Observed risk:** common browser URL copy supplies both `text/plain` and `text/uri-list`; the prior card filter accepted plain text alone and routed this pair to native paste.
- **Fix and proof:** accept the pair only when its URI list matches the plain URL, while files, rich content, mismatched URLs and non-URL text retain native paste. A focused regression failed before the change; the paste suite passes 24/24.
- **Limit:** this exact clipboard format has focused test proof, not a browser paste pass.
...
- **Current state:** S→XL visibly enlarged the label inside a selected rectangle in a disposable board, and XL remained after reload. S (local evidence: geo-label-small-dark.png), XL (local evidence: geo-label-xl-dark.png). A newer Safari rectangle/ellipse pass showed the label size change while a separately chosen stroke-width setting stayed fixed. Without that override, native tldraw Size still sets its default stroke width.
