...
| Slides | Earlier named flows pass; new presentation follow-up pending | Reorder, undo, reload, fullscreen/Escape, rename cancel passed before the latest presentation-stage work | New stage behavior needs a fresh browser pass; no speaker notes, transitions or live audience sharing claim |
...
| Imported diagram arrows | Open visual defect | Current screenshot shows an arrow detached from its node and a stroke crossing a node label | Fix and visible reload proof pending; parser tests do not prove connector geometry |
...
- [ ] **Keep imported connections attached to their native nodes and outside node labels.**
...
- **Required proof:** insert the affected diagram in a visible browser, inspect endpoints and label clearance, move a connected node, and hard reload. The bindings and readable geometry must persist. Focused parser or shape-count tests alone do not close this visual issue.
